...
public class Promocoes {
    ...
    public int verificarPromo(int quantCompra, int quantAluguel, String tipoUser, String tempo, int resul){
        int classif = 0; // Nada de promocoes
        if(quantCompra>=5){//Gratis 1 livro
         classif += 1;   
        }else if(quantCompra>=10 && resul == 1){//Geek 1 mes com compra
         classif += 2;  
        }else if(quantCompra>=20 && resul == 1){// Premium 3 meses
         classif += 3;    
        }else if(quantAluguel>=10 && resul == 1){//Geek 1 mes
         classif += 4;   
        }else{
            classif = classif;
        }
